...
let User = function(data, getAvatar) {
    this.data = data //storing users input
    this.errors = []
...
User.prototype.validate = function(){
    return new Promise(async (resolve, reject) => {
        ...
        if (this.data.username != "" && !validator.isAlphanumeric(this.data.username)) {this.errors.push("Username can only contains letters and nubmers.")}
        ...
        if (this.data.username.length > 0 && this.data.username.length < 4) {this.errors.push("Username must be atleast 3 characters")}
    ...
    return new Promise((resolve, reject) => {
        ...
        usersCollection.findOne({username: this.data.username}).then((attemptedUser) => {
            if (attemptedUser && bcrypt.compareSync(this.data.password, attemptedUser.password)) {
                ...
                reject("Invalid username / passowrd")
            }
        }).catch(function(){
...
User.prototype.register = function () {
    return new Promise(async (resolve, reject) => {
        ...
        //Only if there are no validation errors then save it in db
